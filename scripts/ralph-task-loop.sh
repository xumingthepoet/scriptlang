...
PROMPT_ROUND_1="阅读任务文档 \`${TASK_DOC_ABS}\`，把它作为唯一目标，以 ralph 模式持续推进。你在一个循环里工作：每一轮都按文档里的当前进度执行下一条任务，直接动手，不要跳步，不要空转，不要偏离任务去做无关整理。遵守仓库里的 AGENTS.md；如果本轮改动涉及 crates/ 或会影响 crate 行为、公共接口、测试结果、编译流程，就同步更新 IMPLEMENTATION.md，并在准备把这一轮视为完成前跑通 make gate。"
