...
### 2026-03-23 17:55: Goal 1 Round 3 完成 - 重复逻辑清理和过度抽象消除

**本轮工作：**

**Goal 1 重复逻辑清理：**

1. **`string_attr` → `attr` 统一**
   - `expand/mod.rs` 中的 `string_attr` 与 `form.rs` 中的 `attr` 功能完全相同
   - 删除 `expand/mod.rs::string_attr`，将 5 个调用方改为使用 `crate::semantic::attr`
   - 影响文件：`dispatch.rs`, `quote.rs`, `declared_types.rs`, `module_reducer.rs`

2. **`ExpandRegistry` 零大小结构体扁平化**
   - `ExpandRegistry` 是零大小类型，仅用作方法命名空间
   - 所有方法（`expand`, `dispatch`, `has_builtin_rule`）均不使用 `self`
   - 转为独立 free functions：`dispatch_rule`, `has_builtin_rule`, `expand_form_items`
   - clippy 发现 `dispatch_rule` 中两个 if 分支均返回 `MacroHook`，用 `||` 合并消除重复

**Goal 1 过度抽象消除：**

3. **删除死模块 `values.rs`**
   - `CtValue` 已通过 `pub use ast::*;` 在 `macro_lang/mod.rs` 中导出
   - `values.rs` 的 `pub use super::ast::CtValue` 是完全冗余的重新导出
   - 删除文件和 `mod values;` 声明

4. **删除未使用的 `_scope` 参数**
   - `evaluate_macro_items` 签名中有 `_scope: ExpandRuleScope`，从未被使用
   - 移除参数后更新 `macros.rs` 中的调用点

5. **`LocalScope` 瘦包装内联**
   - `LocalScope` 仅包装 `BTreeSet<String>`，提供 `insert` 和 `contains` 两个 trivial 方法
   - 直接用 `BTreeSet<String>` 替换 `ModuleState.locals` 字段
   - 删除 `LocalScope` 结构体及其 impl（约 15 行）

**Goal 2 验证：**

- USE_MACRO_IMPLEMENTATION_PLAN 全部 9 个 Step 已在历史轮次中完成并记录
- 文档中"最小完成定义"全部满足（见文档末尾 Step 9 记录）
...
**发现的问题：**
- Rust dead_code lint 对模块内使用的私有函数不报警（`eval_stmt`、`eval_expr` 只在模块内调用）
- clippy `if_same_then_else` 警告在重构 `dispatch_rule` 时暴露了两个原本就相同逻辑的分支（`resolve_macro` 和 `is_macro_in_requires` 都返回 `MacroHook`），用 `||` 合并后消除警告
- `values.rs` 中 `#[allow(unused_imports)]` 的存在说明 re-export 从未被使用（`CtValue` 已通过 `ast::*` 导出）
- `_scope` 参数在函数签名中存在但从未被使用，属于"为未来扩展预留"的 YAGNI 案例

**结论：**
Goal 1 和 Goal 2 均已完成。本轮工作减少了 3 个多余类型/模块/参数，简化了 2 处重复逻辑，代码更紧凑。所有改动已通过 gate 测试。
